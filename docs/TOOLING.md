...
| `Decode-ManyLightsPair.py` | validates a private paired `.bin` and compares PI-marked INPUT candidates versus valid OUTPUT prefix; repeated `--anchor NAME X Y Z` matches known positions within0.45gu. Input validity/color and world conversion remain explicitly separate assumptions. |
