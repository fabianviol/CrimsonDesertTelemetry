...
Crimson Desert Telemetry is an independent, open-source, read-only telemetry layer for Crimson Desert. It exposes player and render-camera state as newline-delimited JSON so lighting systems, overlays, accessibility tools, and other community projects can consume the same neutral interface.
...
dotnet run --project .\src\CrimsonDesertTelemetry.Cli -- track 0 100
```

`track` accepts an optional sample count (`0` means unlimited) and interval in milliseconds. Status messages go to stderr; stdout contains JSON Lines only and can be piped directly to another program or recorded:

```powershell
dotnet run --project .\src\CrimsonDesertTelemetry.Cli -- track 0 100 > telemetry.jsonl
```

Example output (values shortened):

```json
{"schemaVersion":"1.0","gameBuild":"24994088","sequence":0,"timestamp":"2026-08-29T20:00:00Z","player":{"position":{"x":-11157.0,"y":761.3,"z":-5969.5}},"camera":{"position":{"x":-11155.0,"y":763.0,"z":-5963.8},"up":{"x":0.0,"y":1.0,"z":0.0},"right":{"x":1.0,"y":0.0,"z":0.0},"forward":{"x":0.0,"y":0.0,"z":1.0},"nearPlane":0.2,"farPlane":3.4028235E+38,"fieldOfViewDegrees":50.0,"aspectRatio":1.775,"consensusCopies":48,"validCopies":100,"distinctStates":7,"rediscovered":false}}
```
...
dotnet publish .\src\CrimsonDesertTelemetry.Cli -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -o .\artifacts\win-x64
```
