...
The external host intentionally opens the game process with query and read permissions only. The optional ASI bootstrap is loaded by a user-installed ASI loader for automatic host startup/lifetime management; it does not patch game code or write game memory. Game-memory writes, arbitrary code injection features, anti-cheat bypasses, and competitive advantages are outside its scope.
