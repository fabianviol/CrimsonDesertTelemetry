...
Reuses the established CPU GI model, explicitly without claiming GPU-CB pairing.
...
def decode(source, assume_layout=False):
    if source.get('format') != 'private-spatial-readback-v1' or source.get('executableSha256') != EXE:
    ...
    if (r.get('status') != 'gpu-complete-texture-only' or r.get('gpuCompleted') is not True or
