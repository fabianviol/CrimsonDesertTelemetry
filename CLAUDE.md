...
- Ambient dimming under cover is established. Three retained R16 payloads reproduce a clear / blocked / clear Variant A sequence, but the earlier 19-trace claim conflicts with its 22-copy table and only one payload per pose remains; repeatability is open. The native HUD owns the next bounded check. Keep t224, engine-raymarch reconstruction and DXR closed. CrimsonHue owns device policy and perceptual output mapping, not telemetry acquisition.
