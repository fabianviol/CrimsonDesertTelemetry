...
- Build 25246367: lights, ambient and occlusion are ALL verified live after the update. Four native anchors moved by +0x21C0 and two managed build gates had to go with them; details and the machine's current state are at the top of docs/HANDOVER.md. Hardcoded anchors live in `ambient_probe.h` and `spatial_probe.cpp` and are not generated from a build definition, so check them with `scripts/Verify-AmbientAnchors.py` after any game update.
- The next real step is CrimsonHue: the ambient estimate needs a perceptual curve with a floor (the indoor value is exactly zero), and the occlusion gate needs hysteresis (verdicts near the boundary flip on hundredths of a game unit). Keep t224, engine-raymarch reconstruction, DXR and new PIX work closed.
