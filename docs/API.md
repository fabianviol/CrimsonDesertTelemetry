...
| `sourceVisibility` | Optional geometric estimate: `clear`/`blocked`/`unknown`. Private physics.6 adds `method=physics-ray-fan` with sampled path counts, actual measured camera/capture and age; physics.7 batches up to256 nearby positions within a shared time budget. No SDF volume or optical transmission %. Raw light records stay intact. Legacy SDF metadata remains supported separately. See [complete contract](SOURCE_VISIBILITY.md). |
